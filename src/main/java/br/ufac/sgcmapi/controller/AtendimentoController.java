...
public class AtendimentoController implements ICrudController<Atendimento> {
    ...
    @GetMapping("/busca/{profissional}/{data}")
    public ResponseEntity<List<String>> getByAgendados(@PathVariable("profissional") Long profissional,@PathVariable("data") Date data) {
        List<String> registros = servico.getByAgendados(profissional,data);
        return new ResponseEntity<>(registros, HttpStatus.OK);
    }
